...
(function () {
  ...
  function effectNote(lowG, highG, highName) {
    ...
    if (highG.med > lowG.med) return `<p>At this station, the <b>${highName}</b> group's typical estimate is <b>higher</b> (${fmt(highG.med)} versus ${fmt(lowG.med)}). Same question, different starting number, different answers. That gap <b>is</b> the anchoring effect.</p>`;
  ...
  DPT.register({
    ...
    steps: [
      ...
      },
      {
        ...
        html: `${ui.study({ name: 'Tversky & Kahneman (1974)', rows: [
            ...
            ['Findings', 'Median estimate: <b>512</b> (ascending) versus <b>2,250</b> (descending). The true answer is <b>40,320</b>.'],
          ...
          ${ui.study({ name: 'Strack & Mussweiler (1997)', rows: [
            ...
            ['Findings', 'Mean estimate: <b>50.1</b> years after the anchor 9 versus <b>66.7</b> years after the anchor 140. (He died at 78.)'],
    ...
    host: {
      checklist: [
        { point: 'Reveal the two groups', hint: 'Your guest was randomly put in one group. The other group saw the numbers in the <b>opposite order</b> (and 9 versus 140 for Gandhi). Same question, different anchor.' },
        ...
        { point: 'The two classic studies', hint: 'Tversky &amp; Kahneman (1974): median 512 versus 2,250. Strack &amp; Mussweiler (1997): mean 50.1 (anchor 9) versus 66.7 (anchor 140).' },
        ...
        { point: 'Real life', hint: 'Englich et al. (2006): legal professionals rolled loaded dice (3 or 9); rolling 9 led to about 8 months’ sentence versus 5. Also market bargaining, menus, “was £200, now £99”.' },
    ...
    concepts: [
      { name: 'Causality', html: 'This station is a true experiment. The laptop randomly put each visitor into the ascending or descending group (and the 9 or 140 group), so the anchor is the only thing that differs systematically between the groups. That is what lets us say the anchor <b>caused</b> the difference in estimates. In real life, such as house sales, we can’t randomise asking prices, so a link between asking price and final price could also be explained by the house simply being better.' },
      { name: 'Measurement', html: 'Anchoring is measured as the difference between the groups’ typical estimates, not as anything one person can report. Tversky and Kahneman used medians and Strack and Mussweiler used means. This station uses medians because one wild guess (say, 10,000,000) would drag a mean miles away. Estimating Gandhi’s age with nothing at stake is also a long way from real decisions, which limits ecological validity.' },
      { name: 'Responsibility', html: 'If a random number on a pair of dice can lengthen a prison sentence (Englich et al., 2006), the legal system has a responsibility to guard against anchors, for example through training or sentencing guidelines. Marketers also use anchors deliberately: “was £200, now £99”. Consumer law in many countries says a “was” price must be a genuine earlier price, precisely because the anchor affects what people will pay.' },
      { name: 'Perspective', html: 'There is more than one explanation of the same result. Tversky and Kahneman saw it as anchoring and insufficient adjustment. Strack and Mussweiler argued instead for selective accessibility: comparing Gandhi with the anchor brings to mind facts that fit that number. Most of the classic studies also used Western students, although large projects across several countries have since found the effect too.' },
    ],
    debate: {
      title: 'Does any random number anchor us, or mainly in classic lab questions?',
      sideA: { label: 'Strong and reliable: Many Labs 1 (Klein et al., 2014)', html: 'A team of researchers repeated classic findings in 36 samples in several countries, with more than 6,000 participants. The standard anchoring questions (for example, “Is Mount Everest taller or shorter than a given height? How tall is it?”) produced large anchoring effects in nearly all of the samples. It was one of the strongest results in the whole project.' },
      sideB: { label: 'Weaker than claimed: Maniadis, Tufano & List (2014)', html: 'Ariely, Loewenstein and Prelec (2003) reported that students who wrote down the last two digits of their personal identification number went on to offer more money for everyday products. Maniadis and colleagues repeated a version of this study and found a much smaller anchoring effect that was not statistically reliable.' },
      why: [
        { factor: 'What is being judged', html: 'Many Labs asked for estimates of facts people don’t know (the height of Everest), where a starting point is really needed. The price studies asked how much people would <b>pay</b>, and people may already have a rough sense of what a product is worth to them, leaving less room for an anchor.' },
        { factor: 'Sample size', html: 'The original price study used a fairly small group of business students at one university. Many Labs had thousands of people across dozens of sites, so its result is much less likely to be a fluke.' },
        { factor: 'Publication bias', html: 'A surprising result, such as “your ID number changes what you pay”, is exciting and gets published and cited. Attempts that find a weak effect are less likely to appear, so striking versions of anchoring may look stronger than they are.' },
        { factor: 'Operationalisation', html: 'Both groups of researchers called it “anchoring”, but the anchor, the task and the stakes were different. In the price studies the number came from participants’ own ID digits, which are obviously unrelated to the product, and people could end up really paying. Small differences like these can change how strongly a number pulls on a judgement.' },
        { factor: 'Different samples', html: 'Many Labs ran the same procedure in many countries and settings, which tests whether the effect generalises. A single study in one place can’t show that, and a repeat in a different place may give a different answer for reasons that have nothing to do with anchoring.' },
      ],
      trust: 'For the classic form of anchoring, where people compare a quantity with a number and then estimate it, the evidence is very strong: a large, multi-site replication found it in nearly every sample. That is the form used at this station. Claims that <b>any</b> random number shifts what people are willing to pay are less secure, because they rest on smaller studies and repeats have produced weaker results. So the fair conclusion is that anchoring is real and robust, but its size depends on the task, and single striking studies (including the exact numbers from Strack and Mussweiler’s Gandhi question) should be treated with some caution.',
      ask: 'A shop wants to use anchoring by showing a random large number on a screen by the till. Based on the two sides of this debate, how well do you think it would work, and why?',
    },
    quiz: {
      core: [
        ...
        { q: 'What did <b>Strack &amp; Mussweiler (1997)</b> show with the Gandhi question?', a: 'Even obviously implausible anchors (9 or 140) shifted people’s estimates of his age', d: ['Anchors only work when they are close to the true answer', 'People ignored the anchor once they realised it was absurd', 'Only the high anchor had any effect on the estimates'], why: 'Mean estimates were 50.1 (anchor 9) versus 66.7 (anchor 140), although both anchors were clearly wrong.' },
        ...
        { q: 'What did the <b>Many Labs 1</b> project (Klein et al., 2014) find about anchoring?', a: 'Classic anchoring effects replicated strongly across dozens of samples in several countries', d: ['Anchoring effects disappeared when the study was repeated in large samples', 'Anchoring was only found in the original American university samples', 'Anchoring worked for high anchors but never for low anchors'], why: 'With more than 6,000 participants in 36 samples, the standard anchoring questions gave large, consistent effects. It was one of the most robust results in the project.' },
      ],
      extra: [
        { q: 'In real house sales, homes with higher asking prices tend to sell for more. Why does this <b>not</b> prove that anchoring is at work?', a: 'Better houses get both higher asking prices and higher sale prices, so quality could explain both', d: ['House buyers are experts, so anchoring can never affect them at all', 'Sale prices are always decided before the asking price is set', 'The link would only count if asking prices were written in words'], why: 'This is a correlation. Without random assignment of asking prices, a third variable (the house itself) could explain the link. Experiments solve this problem.' },
        { q: 'In <b>Englich, Mussweiler &amp; Strack (2006)</b>, what happened after legal professionals rolled loaded dice?', a: 'Those who rolled 9 gave longer sentences than those who rolled 3 (about 8 versus 5 months)', d: ['Those who rolled 3 gave longer sentences than those who rolled 9', 'The dice had no effect because they were experts', 'Those who rolled 9 found the shoplifter not guilty'], why: 'A number that was clearly random still anchored a serious decision made by experienced professionals.' },
